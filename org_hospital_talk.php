        ...
        <form method="POST">
            <div class="relative sm:mb-0 flex-grow w-full">
                <label for="full-name" class="leading-7 text-sm text-gray-400">Chat List</label>
                    <select name="hospital" class="w-full bg-gray-800 bg-opacity-40 rounded border border-gray-700 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-900 focus:bg-transparent text-base outline-none text-gray-100 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        <?php foreach($result as $r){?>
                            <option class="w-full bg-gray-800 bg-opacity-40 rounded border border-gray-700 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-900 focus:bg-transparent text-base outline-none text-gray-100 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"><?php echo $r['Hospital_name'];?></option> 
                        <?php }?>  
                    </select>   
                </div>

                <p>
                <button name="connect" type="Submit" class="text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">Connect</button>
                </p>
                <p>
                
                <?php if (($_SESSION != null) AND isset($_REQUEST['connect'])){?>
                    <div class="container">
